...
class AppProfiler extends EventEmitter {
  private adb: any;
  ...
  async startCapture() {
    ...
    return await new B(async (_resolve, _reject) => {
      ...
      if (
        _.isNumber(this.deviceInfo.api_level) &&
        this.deviceInfo.api_level >= 28
      ) {
        cmd.push('-m', '20');
      }
  ...
  private getSystemCpuUsage(logLine: string) {
    ...
    let out = 0;
    ['user', 'nice', 'sys', 'iow', 'irq', 'sirq', 'host'].forEach(
      (type: string) => {
        let match = logLine.match(new RegExp(`([0-9]{0,})%${type}`));
  ...
  private getSystemMemoryUsage(logLine: string) {
    ...
    if (!match) {
      ...
      return match && match.length > 1
        ? Math.ceil(parseFloat(match[1]) * 1024 * 1024)
        : 0;
    }
